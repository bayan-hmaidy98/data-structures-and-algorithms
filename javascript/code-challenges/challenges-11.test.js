...
function transformToLis(obj) {
  ...
  let osaid = keyArr.map((item, index) => {
    return `<li>${item}: ${valueArr[index]}</li>`;
  });
  return osaid;
}
...
const count = (target, input) => {
  let number = input.reduce((acc, item) => {
    ...
    } else if (typeof item == "number" && item == target) {
      return acc++;
    }
    return acc;
  }, 0);
